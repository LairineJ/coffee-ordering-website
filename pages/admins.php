...
<head>
  ...
  <title>Cafe Haraya | Administrators</title>
</head>
  ...
  <div class="grid-container">
    ...
    <div class="main-title d-flex">
        ...
        <div class="d-flex justify-content-end align-items-center">
            ...
            <button class="btn btn-success d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#addAdmin" style="padding: .7rem; height: 2.4rem;"><i class="fas fa-plus" style="margin-right: 5px;"></i>Add Admin</button>
        </div>
    </div>
    <div id="container">
    ...
    <table id="adminTable" class="table table-striped" style="width:100% table-layout: fixed;" >
        ...
        <?php
            ...
            while($row = mysqli_fetch_assoc($result)) {
                ...
                <!-- Edit Admin Modal -->
                <div class="modal fade" id="editAdminModal_<?php echo $row['admin_id'] ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<?php } ?>
    ...
    <div class="modal fade" id="addAdmin" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <?php include '../assets/php/modals/addAdminModal.php';?>
    </div>
        </div>
    <!-------------------------- End Main -------------------------->
    </main>
        ...
        $.ajax({
            ...
            success: function (html) {
                ...
                $('#selectAllCheckboxes').on('change', function () {
                    ...
                    $('input[name="admin_checkbox[]"]').prop('checked', isChecked);

                    // Trigger DataTables to redraw
                    dataTable.rows().invalidate().draw();
                });

                $('input[name="admin_checkbox[]"]').on('change', function () {
                    // Check if all checkboxes are checked
                    const allChecked = $('input[name="admin_checkbox[]"]:checked').length === $('input[name="admin_checkbox[]"]').length;
