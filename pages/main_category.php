...
<head>
  ...
  <title>Cafe Haraya | Main Category</title>
</head>
...
<body id="body">
  <div class="grid-container">
    ...
    <div class="main-title d-flex">
        <p class="font-weight-bold">MAIN CATEGORY</p>
        <div class="d-flex justify-content-end align-items-center">
            ...
            <button class="btn btn-success d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#addMainCategoryModal" style="padding: .7rem; height: 2.4rem;"><i class="fas fa-plus" style="margin-right: 5px;"></i>Add Main Category</button>
        </div>
    </div>
    <div id="container">
...
<?php } ?>
    ...
    <div class="modal fade" id="addMainCategoryModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <?php include '../assets/php/modals/addMainCategoryModal.php';?>
    </div>
        </div>
    <!-------------------------- End Main -------------------------->
    </main>
  </div>
  <?php include "crudMainCateg.php"?>
        ...
        $.ajax({
            ...
            success: function (html) {
                ...
                $('#selectAllCheckboxes').on('change', function () {
                    ...
                    $('input[name="mCateg_checkbox[]"]').prop('checked', isChecked);

                    // Trigger DataTables to redraw
                    dataTable.rows().invalidate().draw();
                });

                $('input[name="mCateg_checkbox[]"]').on('change', function () {
                    // Check if all checkboxes are checked
                    const allChecked = $('input[name="mCateg_checkbox[]"]:checked').length === $('input[name="mCateg_checkbox[]"]').length;
